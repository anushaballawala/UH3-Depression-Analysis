...
function [outputdir,montageInfo,metadata,data,good_ch_idx,good_ch_labels] = Preprocessing_setup_15s_stim_rVCVS_DBSTRD002(subjecttype,PatientID,experiment_type,hemi,DBStarget,run)
%% load data

%Stim15s_lSCC_f130
neuraldatafolder = sprintf('E:/%s/%s/Raw Data/Stim15s_%s%s_%s',subjecttype, PatientID, hemi,DBStarget,run);
neuraldatafile = sprintf('%s/sub-%s_task-Stim15s_run-%s%s_blk-%s_rawData',neuraldatafolder,PatientID, hemi,DBStarget,run); 
...
%load good channels 
chinfo = load(sprintf('E:/%s/%s/Experiments/lSCC_f130_goodch.mat',subjecttype,PatientID)); 
