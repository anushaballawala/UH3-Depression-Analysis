...
roi_idx = 2; foi_idx = 1; 
%for roi_idx = 1:numel(roi_labels_new)
    
    %for foi_idx = 1:numel(foi_labels_new)
        figure() 
        % run loop for right.
        ...
        % BASELINE.
        % VCVS. 
        h1 = raincloud_plot(vcvs_d, 'box_on', 1, 'color', VCVS_color, 'alpha', 0.6,...
            'box_dodge', 1, 'box_dodge_amount', .24, 'dot_dodge_amount', .24,...
            'box_col_match', 1, 'line_width_density', 0.75, 'bxfacecl',VCVS_color, 'line_width',1.25,...
            'lwr_bnd', 4);
        % SCC. 
        h2 = raincloud_plot(scc_d, 'box_on', 1, 'color', SCC_color, 'alpha', 0.6,...
            'box_dodge', 1, 'box_dodge_amount', 0.67, 'dot_dodge_amount', .67,...
            'box_col_match', 1, 'line_width_density', 0.75, 'bxfacecl', SCC_color, 'line_width',1.25,...
            'lwr_bnd', 4);
                           
        legend([h1{1} h2{1}], {'rVCVS','rSCC'});
        ...
        filename_right = sprintf('%s_%s_right_dbs_raincloud_%s_%s.svg',PatientID,date,roi_labels_new{roi_idx},foi_labels_new{foi_idx});
...
%       plot for left. 
        figure() 
        % VCVS. 
        h1 = raincloud_plot(vcvs_d_left, 'box_on', 1, 'color', VCVS_color, 'alpha', 0.7,...
            'box_dodge', 1, 'box_dodge_amount', .24, 'dot_dodge_amount', .24,...
            'box_col_match', 1, 'line_width_density', 0.75, 'bxfacecl',VCVS_color, 'line_width',1.25,...
             'lwr_bnd', 4);
        % SCC. 
        h2 = raincloud_plot(scc_d_left, 'box_on', 1, 'color', SCC_color, 'alpha', 0.7,...
            'box_dodge', 1, 'box_dodge_amount', 0.67, 'dot_dodge_amount', .67,...
            'box_col_match', 1, 'line_width_density', 0.75, 'bxfacecl', SCC_color, 'line_width',1.25,...
             'lwr_bnd', 4);
                           
        legend([h1{1} h2{1}], {'lVCVS','lSCC'});
        ...
        filename_left = sprintf('%s_%s_dbs_raincloud_%s_%s.svg',PatientID,date,roi_labels_new{roi_idx},foi_labels_new{foi_idx});
