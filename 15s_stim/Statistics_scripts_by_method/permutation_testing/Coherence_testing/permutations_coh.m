function [t_stat,shuffled_labels] = permutations_coh(data,labels,num_perm,fxn_handle) 
...
%             if any(isnan(t(:))) %comment from troubleshooting 04/13 with

%                 keyboard
%             end 
            t_stat(k,:) = reshape(t,1,1,numel(t)); % M x numch x num freqbands
            disp(k)
    end
