...
for i=1:num_ch
    for j = 1:num_trials 
    %prestim 
      [S_prestim(j,i,:),f_prestim,Serr_prestim(:,:,i)] = mtspectrumc(squeeze(permute(indivtr_pre_stim_data(j,i,:),[3,1,2])),params);
         [S_stim1(j,i,:),f_stim1,Serr_stim1(:,:,i)] = mtspectrumc(squeeze(permute(indivtr_stim1_data(j,i,:),[3,1,2])),params);
        [S_stim2(j,i,:),f_stim2,Serr_stim2(:,:,i)] = mtspectrumc(squeeze(permute(indivtr_stim2_data(j,i,:),[3,1,2])),params);
        [S_stim3(j,i,:),f_stim3,Serr_stim3(:,:,i)] = mtspectrumc(squeeze(permute(indivtr_stim3_data(j,i,:),[3,1,2])),params);  
...
avg_pow_prestim = squeeze(mean(pow_prestim,1)); 
avg_pow_stim1 = squeeze(mean(pow_stim1,1)); 
avg_pow_stim2 = squeeze(mean(pow_stim2,1)); 
avg_pow_stim3 = squeeze(mean(pow_stim3,1)); 
avg_pow_poststim = squeeze(mean(pow_poststim,1)); 
