...
if strcmp(PatientID, 'DBSTRD003')
    %downsample data
    stimsync = decimate(double(NS5.Data(4,:)),fs_orig / fs_new);
    cerestim = decimate(double(NS5.Data(9,:)),fs_orig / fs_new);
else
...
figdir = sprintf('/gpfs/data/dborton/TRD_Project/DBSTRD/%s/Experiments/15s_stim/Figures/FindPeaks_Timestamps_%s.png',PatientID,stim_info); 
saveas(gcf, figdir)
timestamps = locs; 
